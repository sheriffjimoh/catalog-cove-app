...
import { TrendingUp, TrendingDown, Eye, MessageCircle, Store, Share2, Users, Award, Calendar } from 'lucide-react';
...
const VendorAnalyticsDashboard = ({ business, analytics, date_range }: Props) => {
    ...
    const calculateTrend = (type: 'store_visits' | 'product_views' | 'inquiries'): number => {
        ...
        const lastWeekTotal = lastWeek.reduce((sum, date) => 
            sum + (analytics.daily_trends[date]?.[type] || 0), 0
        );
        const previousWeekTotal = previousWeek.reduce((sum, date) => 
            sum + (analytics.daily_trends[date]?.[type] || 0), 0
        );
    ...
    const maxValue = Math.max(
        ...chartData.flatMap(d => [d.store_visits, d.product_views, d.inquiries])
    );
...
console.log('Max Value:', maxValue);
            ...
            <div className="max-w-7xl mx-auto p-4 md:p-8 space-y-8">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div>
                        ...
                        <p className="text-slate-600">
                            Track your store performance for <span className="font-semibold text-purple-700">{business.name}</span>
                        </p>
                    </div>
                    ...
                    <div className="flex gap-2">
                        {['7', '30', '90'].map((days) => (
                            <Button
                                ...
                                className={selectedRange === days ? 'bg-purple-700 hover:bg-purple-800' : 'hover:bg-purple-50'}
                            >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
                    ...
                    <Card className="border-0 shadow-lg hover:shadow-xl transition-shadow">
                        <CardHeader className="pb-3">
                            <div className="flex items-center justify-between">
                                ...
                                <Badge variant={calculateTrend('store_visits') >= 0 ? 'default' : 'secondary'} 
                                       className={calculateTrend('store_visits') >= 0 ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}>
                                    {calculateTrend('store_visits') >= 0 ? (
                                        <TrendingUp className="w-3 h-3 mr-1" />
                                    ) : (
                                        <TrendingDown className="w-3 h-3 mr-1" />
                                    )}
                                    {Math.abs(calculateTrend('store_visits')).toFixed(0)}%
                                </Badge>
                            </div>
                        </CardHeader>
                        <CardContent>
                            ...
                            <p className="text-sm text-slate-600">Store Visits</p>
                        </CardContent>
                    </Card>
                    ...
                    <Card className="border-0 shadow-lg hover:shadow-xl transition-shadow">
                        <CardHeader className="pb-3">
                            <div className="flex items-center justify-between">
                                ...
                                <Badge variant={calculateTrend('product_views') >= 0 ? 'default' : 'secondary'} 
                                       className={calculateTrend('product_views') >= 0 ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}>
                                    {calculateTrend('product_views') >= 0 ? (
                                        <TrendingUp className="w-3 h-3 mr-1" />
                                    ) : (
                                        <TrendingDown className="w-3 h-3 mr-1" />
                                    )}
                                    {Math.abs(calculateTrend('product_views')).toFixed(0)}%
                                </Badge>
                            </div>
                        </CardHeader>
                        <CardContent>
                            ...
                            <p className="text-sm text-slate-600">Product Views</p>
                        </CardContent>
                    </Card>
                    ...
                    <Card className="border-0 shadow-lg hover:shadow-xl transition-shadow">
                        <CardHeader className="pb-3">
                            <div className="flex items-center justify-between">
                                ...
                                <Badge variant={calculateTrend('inquiries') >= 0 ? 'default' : 'secondary'} 
                                       className={calculateTrend('inquiries') >= 0 ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}>
                                    {calculateTrend('inquiries') >= 0 ? (
                                        <TrendingUp className="w-3 h-3 mr-1" />
                                    ) : (
                                        <TrendingDown className="w-3 h-3 mr-1" />
                                    )}
                                    {Math.abs(calculateTrend('inquiries')).toFixed(0)}%
                                </Badge>
                            </div>
                        </CardHeader>
                        <CardContent>
                    ...
                    <Card className="border-0 shadow-lg hover:shadow-xl transition-shadow">
                        ...
                        <CardContent>
                            ...
                            <p className="text-sm text-slate-600">Product Shares</p>
                        </CardContent>
                    </Card>
                ...
                <Card className="border-0 shadow-lg bg-purple-700 text-white">
                    <CardContent className="p-6">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-purple-100 mb-2">Conversion Rate</p>
                                <h3 className="text-4xl font-bold">{analytics.conversion_rate}%</h3>
                                <p className="text-sm text-purple-100 mt-2">
                                    {analytics.overview.inquiries} inquiries from {analytics.overview.product_views} product views
                                </p>
                            </div>
                ...
                <Card className="border-0 shadow-lg">
                    <CardHeader>
                        <CardTitle className="text-2xl text-slate-900">Performance Trends</CardTitle>
    ...
    {chartData.map((data, index) => {
        const total = data.store_visits + data.product_views + data.inquiries;
        ...
        return (
            <div key={index} className="flex flex-col items-center" style={{ width: '60px' }}>
                    ...
                    {data.date}
                            ...
                            <div className="flex flex-wrap justify-center gap-6">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-sm text-slate-600">Product Views</span>
                                </div>
                                <div className="flex items-center gap-2">
                ...
                <Card className="border-0 shadow-lg">
                    <CardHeader>
                        <CardTitle className="text-2xl text-slate-900">Top Performing Products</CardTitle>
                        <CardDescription>Your best products by views and inquiries</CardDescription>
                    </CardHeader>
                    <CardContent>
